...
export function AnalyticsDashboard() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-zinc-900 border border-white/10 rounded-2xl p-4 shadow-xl gap-4">
        <div>
          <h2 className="text-xl font-bold flex items-center gap-2"><Target className="w-5 h-5 text-cyan-500"/> Command Center</h2>
          <p className="text-sm text-zinc-400">Filter parameters instantly apply to all downstream intelligence metrics.</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-xs uppercase tracking-widest text-zinc-500 font-bold">Horizon:</span>
            <Select value={timeRange} onValueChange={setTimeRange}>
              <SelectTrigger className="w-[140px] bg-black/50 border-white/10 h-9">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs uppercase tracking-widest text-zinc-500 font-bold">Sector:</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Active Users */}
        <Card className="bg-zinc-900/50 border-white/5 backdrop-blur-xl relative overflow-hidden group">
          <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
          <CardHeader className="flex flex-row items-center justify-between pb-2 relative z-10">
            <CardTitle className="text-sm font-semibold tracking-wide uppercase text-zinc-400">Active Users</CardTitle>
            <Users className="h-4 w-4 text-cyan-400" />
          </CardHeader>
          <CardContent className="relative z-10">
            <div className="text-3xl font-black text-white">{mockKPIs.activeUsers.toLocaleString()}</div>
        ...
        <Card className="bg-zinc-900/50 border-white/5 backdrop-blur-xl relative overflow-hidden group">
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2 relative z-10">
            <CardTitle className="text-sm font-semibold tracking-wide uppercase text-zinc-400">User Retention</CardTitle>
            <Activity className="h-4 w-4 text-purple-400" />
          </CardHeader>
          <CardContent className="relative z-10">
            <div className="text-2xl font-black text-white">{mockKPIs.returningRatio}</div>
        ...
        <Card className="bg-zinc-900/50 border-white/5 backdrop-blur-xl relative overflow-hidden group">
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2 relative z-10">
            <CardTitle className="text-sm font-semibold tracking-wide uppercase text-zinc-400">Avg Read Time</CardTitle>
            <Zap className="h-4 w-4 text-amber-400" />
          </CardHeader>
          <CardContent className="relative z-10">
            <div className="text-3xl font-black text-white">{mockKPIs.avgEngagement}</div>
        ...
        <Card className="bg-zinc-900/50 border-white/5 backdrop-blur-xl relative overflow-hidden group">
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2 relative z-10">
            <CardTitle className="text-sm font-semibold tracking-wide uppercase text-zinc-400">ARPPU</CardTitle>
            <DollarSign className="h-4 w-4 text-emerald-400" />
          </CardHeader>
          <CardContent className="relative z-10">
            <div className="text-3xl font-black text-white">{mockKPIs.arppu}</div>
        ...
        <Card className="bg-card/50 border-white/5 backdrop-blur-sm">
          ...
          <CardContent className="space-y-4">
            {topArticles.map((article, i) => (
              <div key={article.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-zinc-900/60 rounded-xl border border-white/5 hover:border-cyan-500/30 transition-colors gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs font-black text-cyan-500">#{i + 1}</span>
                    <h4 className="font-bold text-white truncate">{article.title}</h4>
                  </div>
                ...
                <div className="flex items-center gap-2 mt-2 sm:mt-0 opacity-80 hover:opacity-100 transition-opacity">
                   <Button size="sm" variant="outline" className="border-amber-500/30 text-amber-500 hover:bg-amber-500 hover:text-white h-8 px-2 text-xs" onClick={() => handleAction("Feature Frontpage", article.title)}>
                     <Star className="w-3 h-3 mr-1" /> Feature
                   </Button>
                   <Button size="sm" variant="outline" className="border-cyan-500/30 text-cyan-400 hover:bg-cyan-500 hover:text-white h-8 px-2 text-xs" onClick={() => handleAction("Push Notification", article.title)}>
          ...
          <CardContent className="space-y-4">
            {worstArticles.map((article) => (
              <div key={article.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-red-950/20 rounded-xl border border-red-500/20 hover:border-red-500/40 transition-colors gap-4">
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-white truncate mb-1">{article.title}</h4>
                  <p className="text-xs text-zinc-400 flex items-center gap-3">
                    <span>By {article.author}</span>
                    <span className="flex items-center gap-1"><AlertTriangle className="w-3 h-3 text-amber-500"/> {article.issues}</span>
                    <span className="font-mono text-zinc-500">{article.views} views</span>
                  </p>
                </div>
                
                {/* Action Layer */}
                <div className="flex items-center gap-2 mt-2 sm:mt-0">
                   <Button size="sm" variant="outline" className="border-red-500/30 text-red-500 hover:bg-red-500 hover:text-white h-8 px-2 text-xs" onClick={() => handleAction("Flag / Unpublish", article.title)}>
                     <AlertTriangle className="w-3 h-3 mr-1" /> Flag
                   </Button>
                   <Button size="sm" variant="outline" className="border-zinc-500/30 text-zinc-400 hover:bg-zinc-800 hover:text-white h-8 px-2 text-xs" onClick={() => handleAction("Send Feedback Email", article.title)}>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {topCreators.map((creator, idx) => (
              <div key={idx} className="bg-zinc-900/40 p-5 rounded-2xl border border-white/5 relative overflow-hidden group hover:border-purple-500/30 transition-colors">
                ...
                <h4 className="text-lg font-bold text-white mb-1 group-hover:text-cyan-400 transition-colors">{creator.name}</h4>
                <p className="text-xs text-zinc-500 font-bold tracking-widest uppercase mb-4">{creator.type}</p>
                
                <div className="flex items-center gap-4 pt-4 border-t border-white/10">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Revenue Generated</span>
                    <span className="text-emerald-400 font-black">{creator.earnings}</span>
                  </div>
                  <div className="flex flex-col border-l border-white/10 pl-4">
                    <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Articles</span>
                    <span className="text-white font-black">{creator.articles}</span>
                  </div>
                </div>
              </div>
            ))}

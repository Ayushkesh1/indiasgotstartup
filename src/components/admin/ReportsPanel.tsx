    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">Review flagged violations and enforce community standards securely.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4 bg-zinc-950 border-zinc-800 relative overflow-hidden group">
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="text-sm text-zinc-500 font-medium">Total Cases</div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
        ...
        <Button variant={activeTab === "pending" ? "default" : "outline"} onClick={() => setActiveTab("pending")} className={activeTab === "pending" ? "bg-amber-950/50 text-amber-400 border-amber-900" : "bg-transparent border-zinc-800 text-zinc-400"}>
          Pending Triage
        </Button>
        <Button variant={activeTab === "under_review" ? "default" : "outline"} onClick={() => setActiveTab("under_review")} className={activeTab === "under_review" ? "bg-blue-950/50 text-blue-400 border-blue-900" : "bg-transparent border-zinc-800 text-zinc-400"}>
          Under Review
        </Button>
        <Button variant={activeTab === "resolved" ? "default" : "outline"} onClick={() => setActiveTab("resolved")} className={activeTab === "resolved" ? "bg-emerald-950/50 text-emerald-400 border-emerald-900" : "bg-transparent border-zinc-800 text-zinc-400"}>
          Resolved / Dismissed
        </Button>
      </div>
          ...
          <Table>
            <TableHeader className="bg-zinc-900/50">
              <TableRow className="border-b border-zinc-800 hover:bg-transparent">
                <TableHead className="text-zinc-400 font-medium h-12">Violation Reason</TableHead>
                <TableHead className="text-zinc-400 font-medium">Severity</TableHead>
                <TableHead className="text-zinc-400 font-medium">Reported Content</TableHead>
                <TableHead className="text-zinc-400 font-medium">Status Workflow</TableHead>
                <TableHead className="text-right text-zinc-400 font-medium pr-6">Quick Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredReports.length > 0 ? filteredReports.map((report) => (
                <TableRow key={report.id} className="border-b border-zinc-800/50 hover:bg-zinc-900/80 transition-colors">
                  <TableCell>
                     <div className="flex flex-col">
                        <span className="font-medium text-zinc-200 capitalize">{report.reason}</span>
                        <span className="text-[10px] text-zinc-500 mt-1">Logged by: {report.profiles?.full_name || "Anonymous User"}</span>
                        <span className="text-[10px] text-zinc-500 flex items-center gap-1 mt-0.5"><History className="w-3 h-3" /> {formatDistanceToNow(new Date(report.created_at), { addSuffix: true })}</span>
                     </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right pr-6">
                    <div className="flex flex-col gap-1 items-end">
                      <div className="flex gap-2">
                         <Button size="sm" variant="ghost" className="h-7 px-2 text-zinc-400 hover:text-blue-400 bg-zinc-900/50" onClick={() => setSelectedReport(report)} title="View Case Details">
                      ...
                      <Select onValueChange={(action) => {
                        ...
                        <SelectTrigger className="w-[140px] h-6 text-[10px] bg-black border-zinc-800 text-zinc-400 focus:ring-0 mt-1">
                        ...
                        <SelectContent className="bg-zinc-950 border-zinc-800 z-50">
                          ...
                          <SelectItem value="dismiss" className="text-xs text-zinc-500">Ignore / Dismiss</SelectItem>
           ...
           {selectedReport && (
              <>
                <DialogHeader className="border-b border-zinc-800 pb-4">
                  ...
                  <DialogDescription className="text-zinc-500 flex justify-between items-center mt-2">
                ...
                <div className="space-y-6 py-4">
                   ...
                   <div className="bg-black border border-zinc-800 rounded-lg p-4 space-y-3">
                     <h4 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Notice of Violation</h4>
                     
                     <div className="grid grid-cols-2 gap-4">
                        <div>
                          <p className="text-[10px] text-zinc-500 font-mono">Infraction Category</p>
                          <p className="text-sm font-medium text-red-400 capitalize">{selectedReport.reason}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-zinc-500 font-mono">Reported By</p>
                          <p className="text-sm font-medium text-zinc-300">{selectedReport.profiles?.full_name || "Anonymous ID"}</p>
                        </div>
                     </div>
                     <div className="pt-2 border-t border-zinc-800/50">
                        <p className="text-[10px] text-zinc-500 font-mono">Complainant Evidence / Note</p>
                   ...
                   {/* Content Context */}
                   <div className="relative p-4 rounded-lg bg-zinc-900/50 border border-zinc-800 overflow-hidden">
                     <div className="absolute top-0 left-0 w-1 h-full bg-blue-500"></div>
                     <h4 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Subject Content Trace</h4>
                     <h3 className="font-semibold text-lg text-zinc-100">{selectedReport.articles?.title || "Undefined Title"}</h3>
                     <p className="text-sm text-zinc-400 mt-1">This context block shows a localized snapshot of the flagged element.</p>
                ...
                <DialogFooter className="border-t border-zinc-800 pt-4 flex sm:justify-between items-center w-full">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" onClick={() => setSelectedReport(null)} className="h-8 text-xs border-zinc-800 text-zinc-300">Close</Button>

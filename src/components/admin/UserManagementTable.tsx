    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">Manage platform members, track activities, and govern roles securely.</p>
        </div>
          ...
          <Table>
            <TableHeader className="bg-zinc-900/50">
              <TableRow className="border-b border-zinc-800 hover:bg-transparent">
                <TableHead className="text-zinc-400 font-medium h-12">User Identity</TableHead>
                <TableHead className="text-zinc-400 font-medium">Access Permissions</TableHead>
                <TableHead className="text-zinc-400 font-medium">System Status</TableHead>
                <TableHead className="text-right text-zinc-400 font-medium pr-6">Append Role</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredUsers.length > 0 ? filteredUsers.map((user) => {
                ...
                return (
                 <TableRow 
                   key={user.id} 
                   className="border-b border-zinc-800/50 hover:bg-zinc-900/80 cursor-pointer transition-colors"
                  ...
                  <TableCell>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="font-mono text-[10px] text-zinc-500">ID: {user.id.slice(0, 12)}...</span>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-wrap gap-1.5">
                      {user.roles.map((role) => {
                        ...
                        return (
                          <Badge key={role} variant="outline" className={`gap-1.5 text-xs ${badgeColor}`}>
                            ...
                            <button
                              className="ml-1 opacity-50 hover:opacity-100 hover:text-white focus:outline-none"
                      ...
                      {user.roles.length === 0 && (
                        <Badge variant="outline" className="bg-zinc-900/50 text-zinc-500 border-zinc-800">BASE USER</Badge>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right pr-6" onClick={(e) => e.stopPropagation()}>
                    <Select onValueChange={(role) => handleAssignRole(user.id, role)}>
                      ...
                      <SelectContent className="bg-zinc-950 border-zinc-800">
                        ...
                        <SelectItem value="user" className="text-zinc-300 text-xs">Base User</SelectItem>
                      </SelectContent>
                    </Select>
                  </TableCell>
                </TableRow>
            ...
            <>
              <DialogHeader className="flex flex-row items-center gap-4 pb-4 border-b border-zinc-800">
                <Avatar className="h-16 w-16 border-2 border-zinc-800">
                  ...
                  <AvatarFallback className="bg-zinc-900 text-zinc-400 text-xl font-bold">
                    {selectedUser.full_name?.charAt(0) || "U"}
                  </AvatarFallback>
                </Avatar>
                <div>
                  <DialogTitle className="text-2xl text-zinc-100 flex items-center gap-2">
                    {selectedUser.full_name || "Anonymous User"}
                    <CheckCircle2 className="w-5 h-5 text-blue-500" />
                  </DialogTitle>
                  <DialogDescription className="font-mono text-xs mt-1 text-zinc-500">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 py-4">
                ...
                <div className="space-y-4">
                  <h4 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider">Account Metrics</h4>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-zinc-900 border border-zinc-800 p-3 rounded-lg">
                      <FileText className="w-4 h-4 text-zinc-500 mb-2"/>
                ...
                <div className="space-y-4">
                  <h4 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider">Activity Tracking</h4>
                  <div className="bg-black border border-zinc-800 rounded-lg p-4 h-[180px] overflow-y-auto space-y-3">
                    <div className="flex gap-3 items-start relative">
                      ...
                      <div>
                        <p className="text-xs text-zinc-300">Published <span className="text-blue-400">"The Future of FinTech in India"</span></p>
                    ...
                    <div className="flex gap-3 items-start relative">
                      ...
                      <div className="absolute top-3 left-1 w-px h-full bg-zinc-800"></div>
                      <div>
                        <p className="text-xs text-zinc-300">Logged in from <span className="font-mono bg-zinc-900 px-1 rounded text-zinc-400">IPv4 Address</span></p>
                    ...
                    <div className="flex gap-3 items-start">
                      ...
                      <div>
                        <p className="text-xs text-zinc-300">Completed Payout Processing</p>
              ...
              <div className="border-t border-zinc-800 pt-5 mt-2">
                 <h4 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-3">Security & Interventions</h4>
                 <div className="flex flex-wrap gap-2">
                   <Button variant="outline" onClick={() => handleSimulatedAction("Manual Verification", "User Verified Manually")} className="border-blue-900 bg-blue-950/30 text-blue-400 hover:bg-blue-900 hover:text-white h-8 text-xs">
                     <MailCheck className="w-3 h-3 mr-2" /> Verify Account
                   </Button>
                   <Button variant="outline" onClick={() => handleSimulatedAction("Password Reset", "Reset Link Dispatched")} className="border-zinc-800 bg-black text-zinc-300 hover:bg-zinc-800 hover:text-white h-8 text-xs">
                     <KeyRound className="w-3 h-3 mr-2" /> Force Reset Password
                   </Button>
                   <Button variant="outline" onClick={() => handleSimulatedAction("Ban/Suspend", "User Systematically Banned", "destructive")} className="border-amber-900 bg-amber-950/30 text-amber-500 hover:bg-amber-900 hover:text-white h-8 text-xs ml-auto">
                     <Ban className="w-3 h-3 mr-2" /> Ban/Suspend
                   </Button>
                   <Button variant="outline" onClick={() => handleSimulatedAction("Delete", "User Account Permanently Purged", "destructive")} className="border-red-900 bg-red-950/30 text-red-500 hover:bg-red-900 hover:text-white h-8 text-xs">
        ...
        <DialogContent className="bg-zinc-950 border-zinc-800 sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Primary Account (Survivor)</label>
              <Input placeholder="Enter Primary UUID..." className="bg-black border-zinc-800 text-zinc-300 font-mono text-sm" />
            </div>
            <div className="flex justify-center -my-2 relative z-10">
               <div className="bg-zinc-900 border border-zinc-800 rounded-full p-2">
                 <LinkIcon className="w-4 h-4 text-zinc-500 rotate-90" />
               </div>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Secondary Account (Absorbed)</label>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowMergeModal(false)} className="border-zinc-800 text-zinc-300">Cancel</Button>
            <Button 
               onClick={() => {

...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Verify credentials via secure edge function (admin_credentials is no longer publicly readable)
      const { data: result, error } = await supabase.functions.invoke("verify-admin-login", {
        body: { username, password },
      });

      if (error || !result?.success || !result?.admin) {
        toast({
          title: "Access Denied",
